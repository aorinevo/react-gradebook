...
class GradebookTr extends Component<any,any> {
  renderDropDown(){
    return (
      <td key={-4}>
  ...
  renderRow() {
    ...
    let data = this.props.cells.map( (item: any) => <td key={item.id}>{item.assign_points_earned}</td> );
    data.unshift(this.renderDropDown(),
      <td key={-3}>{first_name}</td>, 
      <td key={-2}>{last_name}</td>, 
      <td key={-1}>{user_login}</td>
    );
    return data;
  }
